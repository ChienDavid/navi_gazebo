...
class PurePursuit:
    ...
    def __init__(self, g, d, speed, angle_range):
        ...
        # ROS topics
        self.cmdPub = rospy.Publisher("/tb3_0/cmd_vel", Twist, queue_size=10)
        self.cmd = Twist()

        rospy.Subscriber("/globalpath", Pathmsgs, self.callback_path)
        rospy.Subscriber("/tb3_0/odom", Odometry, self.callback_robot)
        self.check_connection("/globalpath", Pathmsgs)
...
def main():
    ...
    pp_controller = PurePursuit(g, d, target_speed, angle_range=angle_range)
